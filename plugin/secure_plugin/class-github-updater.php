...
class Digipay_GitHub_Updater {
    ...
    public function __construct($plugin_file, $github_username, $github_repo, $version) {
        ...
        $this->cache_key = 'digipay_github_update_' . md5($this->plugin_basename);
    ...
    public function add_check_update_link($links) {
        $check_link = '<a href="' . wp_nonce_url(
            admin_url('plugins.php?digipay_check_update=1'),
            'digipay_check_update'
        ) . '">Check for updates</a>';
    ...
    public function handle_manual_check() {
        if (!isset($_GET['digipay_check_update'])) {
            return;
        }
        
        if (!wp_verify_nonce($_GET['_wpnonce'], 'digipay_check_update')) {
            return;
        }
        ...
        wp_redirect(admin_url('plugins.php?digipay_checked=1'));
        exit;
    }
    ...
    private function get_github_release_info() {
        ...
        if (is_wp_error($response)) {
            error_log('Digipay Updater: GitHub API error - ' . $response->get_error_message());
            return false;
        }

        $code = wp_remote_retrieve_response_code($response);
        if ($code !== 200) {
            error_log('Digipay Updater: GitHub API returned ' . $code);
            return false;
        }
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) [
            'name'          => 'Digipay Payment Gateway',
            'slug'          => $this->plugin_slug,
            'version'       => $release_info->version,
            'author'        => '<a href="https://github.com/' . $this->github_username . '">Digipay</a>',
            ...
            'sections'      => [
                'description' => 'Digipay payment gateway for WooCommerce with automatic updates.',
                'changelog'   => $changelog,
            ],
            'banners'       => [],
        ];
...
add_action('admin_notices', function() {
    if (isset($_GET['digipay_checked'])) {
        echo '<div class="notice notice-success is-dismissible">';
        echo '<p><strong>Digipay:</strong> Update check complete. ';
